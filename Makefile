...
verify:
	@echo "*** DB ***:"
	ssh -l ubuntu $(PREFIX)-db.node.cloud.um.edu.ar 'sudo mysql -uroot webapp_db -e "show tables from webapp_db;"' && echo PASS. || echo FAIL.
	@echo "*** WEB ***:"
	curl -m2 -sL http://$(PREFIX)-web-{1,2}.node.cloud.um.edu.ar/php-mysql | grep Sample.App && echo PASS. || echo FAIL.
	@echo "*** FE ***:"
	curl -m2 -sL http://$(PREFIX)-fe.node.cloud.um.edu.ar/php-mysql | grep Sample.App && echo PASS. || echo FAIL.

bench:
	ab -c10 -n 1000 http://$(PREFIX)-fe.node.cloud.um.edu.ar/php-mysql
